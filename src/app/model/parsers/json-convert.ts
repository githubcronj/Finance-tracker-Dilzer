...
export class JsonConvert {
     ...
     */
    private _ignorePrimitiveChecks: boolean = true;
     ...
     * @see https://www.npmjs.com/package/json2typescript full documentation
     */
    deserialize(json: any, classReference: any): any {
     ...
     * @see https://www.npmjs.com/package/json2typescript full documentation
     */
    deserializeObject(jsonObject: any, classReference: any): any {
        ...
        let discriminatorInfo = classReference.discriminatorInfo
        if (discriminatorInfo) {

            let discriminatorKey = discriminatorInfo.key
            let subclasses = discriminatorInfo.subclasses
            if (discriminatorKey != undefined && subclasses != undefined) {
                let discriminator = jsonObject[discriminatorKey]
                if (discriminator) {
                    for (let key in subclasses) {
                        if (key === discriminator) {
                            let className = subclasses[key]
                            let subClassRef = entities[className]
                            if (subClassRef) {
                                classReference = subClassRef
                            }
                            break;
                        }
                    }
     ...
     * @see https://www.npmjs.com/package/json2typescript full documentation
     */
    deserializeArray(jsonArray: any[], classReference: any): any[] {
